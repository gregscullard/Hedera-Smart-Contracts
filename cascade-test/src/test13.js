const {
    ...
    TokenInfoQuery, DelegateContractId,
...
async function main() {
    ...
    const logicContractId = await createContract(client, logicContractJson, 10000, contractAdminKey);
    ...
    const tokenId = await createToken(client, aliceAccountId, DelegateContractId.fromString(interfaceContractId.toString()))
    ...
    try {
        await executeContract(client, logicContractId, 50000,"mintCallInterfaceCall", functionParameters, contractAdminKey);
    ...
    try {
        await executeContract(client, logicContractId, 50000,"mintCallInterfaceDelegate", functionParameters, contractAdminKey);
        console.log(`-- Minting was successful`);
    } catch (e) {
        console.error(e);
    }
    ...
    try {
        await executeContract(client, logicContractId, 50000,"mintDelegateInterfaceCall", functionParameters, contractAdminKey);
        console.log(`-- Minting was successful`);
    } catch (e) {
        console.error(e);
    }
    ...
    console.log(`Minting via logic which makes a delegate call to interface which calls HTS`);
    try {
        await executeContract(client, logicContractId, 50000,"mintDelegateInterfaceDelegate", functionParameters, contractAdminKey);
        console.log(`-- Minting was successful`);
    } catch (e) {
        console.error(e);
    }
